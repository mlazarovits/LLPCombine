...
//	std::string f = workdir+"fitDiagnostics.MClnUpDnNFitPseudoShape.root";
	...
	std::string f = "fitDiagnostics1PhoDelayed2bin.root";
	...
	std::vector<std::string> bingrid = {"00","10"};
	...
	TFile* fout = new TFile((workdir+"1PhoDelayed2BincombinedBinHists.root").c_str(), "RECREATE");
	fout->cd();
	for(auto hist : hists)
		hist->Write();
	tf->cd();	
